...
class _BookRendererState extends State<BookPlayerRenderer>
          ...
          GestureDetector(
            ...
            onVerticalDragUpdate: (details) {
              ...
              if (widget.dragAnimation == true) {
                double y = (-draggingMoved.dy / 800).clamp(-1, 1);
                progress.value =
                    _clampProgress(startDraggingProgress + (y.abs()) * y.sign);
              }
            },
            onVerticalDragEnd: (details) {
              ...
              if (details.velocity.pixelsPerSecond.dy.abs() > 30) {
                offset = -details.velocity.pixelsPerSecond.dy.sign;
        ...
        children: [
          GestureDetector(
            ...
            onHorizontalDragUpdate: (details) {
              ...
              draggingMoved += details.delta;
              if (widget.dragAnimation == true) {
                double x = (-draggingMoved.dx / 400).clamp(-1, 1);
                progress.value =
  ...
  //           draggingMoved += details.delta;
  //           if (widget.dragAnimation == true) {
  //             double y = (-draggingMoved.dy / 392).clamp(-1, 1);
  //             progress.value = _clampProgress(
  //               startDraggingProgress + (y.abs()) * y.sign,
  //             );
  //           }
  //         },
  //         onVerticalDragEnd: (details) {
